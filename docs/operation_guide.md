...
### Googleフォームで入荷・出荷・棚卸・商品登録を行う場合

Googleスプレッドシート連携（API設定）とフォーム連携をセットアップすると、スキャンせずにスマホのフォームから登録できます。

```
【入荷】
1. 業者から荷物が届く
      ↓
2. スマホで「お守り入荷登録」フォームを開く（管理メニュー → API設定 → Googleフォーム連携 にURLとQRあり）
      ↓
3. 入力者・商品・箱数を入力して送信（1回で最大10商品）
      ↓
4. 在庫・履歴・発注状況に自動反映。1〜2分でラベルPDFがメールで届く
      ↓
5. ラベルを印刷して箱に貼り、倉庫に収納（納品モードのスキャンは不要）

【出荷】
1. 「お守り出荷登録」フォームで入力者・商品・箱数（端数）・出荷先を送信
      ↓
2. 在庫が減り、安心在庫を下回るとメールで通知

【棚卸】
1. 「お守り棚卸登録」フォームで入力者・商品・実在庫数を送信（1回で最大10商品）
      ↓
2. 反映の前に自動でバックアップが取られる
      ↓
3. 送った商品だけ在庫がその数に置き換わる（送っていない商品はそのまま）
      ↓
4. 差異が履歴に記録される。間違えたら管理画面の「バックアップと復元」で戻せる

【リストにない商品が見つかったとき】
1. 「お守り商品登録」フォームで 商品名・商品コード・入数（と今ある個数）を送信
      ↓
2. 商品マスタに追加され、1分ほどで他のフォームの選択肢にも出てくる
```

**棚卸フォームの空欄と 0 は意味が違います。** 空欄＝数えていない（在庫を触らない）、`0`＝在庫ゼロ（0個として反映）。

**商品コードは英大文字のみ 1〜12文字**（例: `KOTSU`）です。数字・小文字・記号は使えません。
QRラベルの読み取りがこの形を前提にしているためで、規則から外れたコードはエラーで差し戻されます。

詳しくは [google_form_guide.md](google_form_guide.md) を参照してください。

### 在庫を上書きする操作とバックアップ

棚卸は在庫を「数えた数」で上書きします。次の場面では、反映の前に自動でバックアップが取られます。

| 操作 | バックアップ | 設定キー |
|---|---|---|
| 棚卸フォームの送信 | 取る | `backupBeforeFormInventory`（既定 TRUE） |
| 管理画面「棚卸し」タブの反映 | 取る | `backupBeforeAppInventory`（既定 TRUE） |
| 管理画面「在庫を保存」 | 取らない | `backupBeforeStockSave`（既定 FALSE） |
| 毎日決まった時刻 | 取らない | `dailyBackup`（既定 FALSE） |

戻すときは 管理メニュー →「API設定」→「バックアップと復元」で、世代を選び
**「変更点を確認」で内容を確かめてから**「この内容で復元」を押します。
復元の直前の状態も控えられるので、復元そのものを取り消せます。
...
例：
HEALTH-25-0001  → 健康守り 2025年 第1箱
MONEY-25-0002   → 金運守り 2025年 第2箱
```
...
|------------------|------------|------------|------------------|--------------|
| HEALTH | 25 | 1 | =A2&"-"&B2&"-"&TEXT(C2,"0000") | =IMAGE("https://quickchart.io/qr?text="&ENCODEURL(D2)&"&size=100") |
