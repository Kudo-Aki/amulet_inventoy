...
2. D列に `=A1&"-"&B1&"-"&TEXT(C1,"0000")` でQRコードテキストを生成
...
## Googleフォーム連携（入荷・出荷・棚卸・商品登録とラベルPDFの自動送付）

Googleスプレッドシート連携（API設定）を使っている場合、Googleフォームから4種類の登録ができます。

- **入荷**: 在庫が「箱数×入数」だけ増え、履歴と発注状況（分納の消し込み）に反映されます。箱に貼るラベル（QR付き）のPDFが入力者宛にメールで届きます
- **出荷**: 在庫が減り、安心在庫を下回るとメールで通知されます
- **棚卸**: 数えた実際の個数を送ると、その商品だけ在庫がその数に置き換わります（送っていない商品はそのまま）。反映の前に自動でバックアップを取ります
- **商品登録**: マスタに無い授与品をその場で登録できます。初期在庫を入れれば登録と在庫の計上が1回で済みます
- フォームで入荷登録した箱は「箱台帳」に記録され、納品モードでスキャンしても二重計上されません
- フォームのURLとQRコードは 管理メニュー → API設定 → 「Googleフォーム連携」で確認できます

### バックアップと復元

在庫を上書きする操作（棚卸）の前に、商品管理シートの内容が自動で控えられます。
管理メニュー → API設定 → 「バックアップと復元」で、世代を選び **変更点を確認してから** 復元できます。
復元の直前の状態も自動で控えられるので、復元そのものを取り消せます。
