...
## Googleフォーム連携（入荷・出荷登録とラベルPDFの自動送付）

Googleスプレッドシート連携（API設定）を使っている場合、Googleフォームから入荷・出荷を登録できます。

- 入荷フォームを送信すると在庫が「箱数×入数」だけ増え、履歴と発注状況（分納の消し込み）に反映されます。箱に貼るラベル（QR付き）のPDFが入力者宛にメールで届きます
- 出荷フォームを送信すると在庫が減り、安心在庫を下回るとメールで通知されます
